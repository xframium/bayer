        ...
        try
        {
            ...
            while ( webDriver == null )
            {
                try
                {
                    ...
                    String cloudUrl = "https://" + URLEncoder.encode( ConfigurationManager.instance().getProperty( "UserName" ), "UTF-8" ) + ":" + URLEncoder.encode( ConfigurationManager.instance().getProperty( "Password" ), "UTF-8" ) + "@"
                            + ConfigurationManager.instance().getProperty( "CloudURL" ) + "/nexperience/perfectomobile/wd/hub";
            ...
            PerfectoExecutionContext perfectoExecutionContext = new PerfectoExecutionContext.PerfectoExecutionContextBuilder().withProject( new Project( testContext.getSuite().getName(), testVersion ) ).withContextTags( tagString.toArray( new String[0] ) )
                    .withWebDriver( webDriver ).build();
            try
            {
